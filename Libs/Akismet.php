...
class Akismet
{
     ...
     */
    public function __construct($blogURL, $wordPressAPIKey,Request $request) {
        ...
        $this->comment['user_agent'] = $request->server->get('HTTP_USER_AGENT');

        if($request->server->get('HTTP_REFERER') !== null) {
            $this->comment['referrer'] = $request->server->get('HTTP_REFERER');
        }
         ...
         * PHP5 one...
         */
        $this->comment['user_ip'] = $request->server->get('REMOTE_ADDR') != getenv('SERVER_ADDR') ? $request->server->get('REMOTE_ADDR') : getenv('HTTP_X_FORWARDED_FOR');
    }
     ...
     * @throws exception
     */
    public function isCommentSpam(Request $request) {
        $response = $this->sendRequest($this->getQueryString($request), $this->wordPressAPIKey . '.rest.akismet.com', '/' . $this->akismetVersion . '/comment-check');
    ...
    public function submitSpam(Request $request) {
        $this->sendRequest($this->getQueryString($request), $this->wordPressAPIKey . '.' . $this->akismetServer, '/' . $this->akismetVersion . '/submit-spam');
    }
     ...
     * @param Request $request
     */
    public function submitHam(Request $request) {
        $this->sendRequest($this->getQueryString($request), $this->wordPressAPIKey . '.' . $this->akismetServer, '/' . $this->akismetVersion . '/submit-ham');
    }
